...
def pre_growth_adjust(
    ...
    with torch.no_grad():
        phi_old = net.encode(data.observations).cpu().numpy()                        # (B, hidden_size)
        target_max, _ = target_net(data.next_observations).max(dim=1)
        y = (data.rewards.flatten() + gamma * target_max * (1 - data.dones.flatten())).cpu().numpy()

    phi_aug = np.hstack([phi_old, np.ones((phi_old.shape[0], 1))])                  # (B, hidden_size + 1)
    wb, _, _, _ = np.linalg.lstsq(phi_aug, y, rcond=None)                          # (hidden_size + 1,)
    w, b = wb[:-1], wb[-1]

    with torch.no_grad():
        net.q_head.weight[:] = torch.tensor(w, dtype=torch.float32, device=device)
        net.q_head.bias[:] = b
