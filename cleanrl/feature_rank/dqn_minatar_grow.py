...
class Args:
    ...
    initial_filters: int = 4
    """initial number of conv filters; grows to 16 over n_growth_steps events"""
    initial_hidden: int = 32
    """initial encoder hidden size; grows to 128 over n_growth_steps events"""
    n_growth_steps: int = 3
    """number of growth events evenly spaced over training"""
    growth_n_batches: int = 1
    """number of replay buffer batches used to compute growth target
        directions
    """
...
class QNetwork(nn.Module):
    def __init__(self, env, n_filters: int = 16, hidden_size: int = 128):
        super().__init__()
        obs_shape = env.single_observation_space.shape  # (H, W, C)
        n_channels = obs_shape[-1]

        self.conv = nn.Sequential(
            nn.Conv2d(n_channels, n_filters, kernel_size=3, stride=1),
            nn.ReLU(),
        )
...
def compute_growth_directions(
    ...
    """
    Compute target activation directions for new neurons via residual SVD.

    1. Draw n_batches independent batches from the replay buffer.
    2. For each batch: compute Bellman residual r̂, project onto the orthogonal
       complement of Φ_old → r̂_⊥.
    3. Stack into R_⊥ ∈ R^{batch_size × n_batches} and SVD.
    4. Return top n_new left singular vectors as target directions,
       plus the last batch's observations for the regression step.

    Returns:
        target_dirs : np.ndarray, shape (batch_size, n_new)
        reg_obs     : torch.Tensor, observations from the last batch
    """
    R_perp_cols = []
    reg_obs = None
    ...
    for _ in range(n_batches):
        data = replay_buffer.sample(batch_size)
        with torch.no_grad():
            phi_old = old_net.encode(data.observations)[:, :feature_split].cpu().numpy()
            target_max, _ = target_net(data.next_observations).max(dim=1)
            td_target = (data.rewards.flatten() + gamma * target_max * (1 - data.dones.flatten())).cpu().numpy()
            q_val = old_net(data.observations).gather(1, data.actions).squeeze().cpu().numpy()
            r_hat = td_target - q_val  # (batch_size,)

        coeff = np.linalg.lstsq(phi_old, r_hat, rcond=None)[0]
        r_perp = r_hat - phi_old @ coeff
        R_perp_cols.append(r_perp)
        reg_obs = data.observations

    R_perp = np.stack(R_perp_cols, axis=1)  # (batch_size, n_batches)
    U, _, _ = np.linalg.svd(R_perp, full_matrices=False)
    return U[:, :n_new], reg_obs  # (batch_size, n_new)


def init_new_neurons(
    new_net: QNetwork,
    old_net: QNetwork,
    target_dirs: np.ndarray,
    reg_obs: torch.Tensor,
    device,
) -> None:
    """
    Set incoming weights of new neurons by regression so that their
    pre-activation outputs on reg_obs approximate target_dirs.

    Step 1 (encoder): flat_X @ W_new ≈ target_dirs
                      → sets encoder[1].weight[old_h:, :]
    Step 2 (conv):    back-project residual through new encoder weights
                      to find target activations for new conv filters,
                      then regress on unfolded input patches
                      → sets conv[0].weight[old_nf:]
    """
    old_h      = old_net.q_head.in_features
    old_nf     = old_net.conv[0].out_channels
    old_flat   = old_net.encoder[1].in_features
    delta_nf   = new_net.conv[0].out_channels - old_nf
    n_channels = new_net.conv[0].in_channels

    with torch.no_grad():
        x_perm  = reg_obs.permute(0, 3, 1, 2).float()
        flat_X  = new_net.encoder[0](new_net.conv(x_perm)).cpu().numpy()
        patches = F.unfold(x_perm, kernel_size=3, stride=1)  # (B, C*9, L)

    # Step 1: regress flat features onto target directions
    W_new, _, _, _ = np.linalg.lstsq(flat_X, target_dirs, rcond=None)

    with torch.no_grad():
        new_net.encoder[1].weight[old_h:, :].copy_(
            torch.tensor(W_new.T, dtype=torch.float32, device=device)
        )

    # if delta_nf == 0:
    #     return

    # # Step 2a: residual that old flat features can't explain through new encoder weights
    # residual_hidden = target_dirs - flat_X[:, :old_flat] @ W_new[:old_flat, :]

    # # Step 2b: invert new encoder weights to get target activations for new flat features
    # # solve W_new[old_flat:].T @ x ≈ residual_hidden.T  to  x shape (new_flat-old_flat, B)
    # target_flat_new_T, _, _, _ = np.linalg.lstsq(
    #     W_new[old_flat:, :].T, residual_hidden.T, rcond=None
    # )
    # target_flat_new = target_flat_new_T.T  # (B, new_flat - old_flat)

    # # Step 2c: regress on unfolded patches to find new conv filter weights
    # patch_size = patches.shape[2]  # L = H_out * W_out
    # P = patches.permute(0, 2, 1).reshape(-1, n_channels * 9).cpu().numpy()  # (B*L, C*9)

    # with torch.no_grad():
    #     for f in range(delta_nf):
    #         t = target_flat_new[:, f * patch_size:(f + 1) * patch_size].reshape(-1)
    #         w, _, _, _ = np.linalg.lstsq(P, t, rcond=None)
    #         new_net.conv[0].weight[old_nf + f].copy_(
    #             torch.tensor(w.reshape(n_channels, 3, 3), dtype=torch.float32, device=device)
    #         )
...
def grow_network(
    ...
    """
    Expand QNetwork from its current size to (new_n_filters, new_hidden).

    Weight initialization rule (function-preserving):
      - existing to existing weights: copied unchanged
      - existing neuron (layer L) to new neuron (layer L+1): random init
      - new neuron (layer L) to existing neuron (layer L+1): 0
      - new neuron (layer L) to new neuron (layer L+1): random init
    Biases follow the same rule (bias = weight on a constant-1 input, always "existing").

    This guarantees network output is identical before and after growth.
    The optimizer must be rebuilt after calling this function.
    """
    new_net = QNetwork(env, n_filters=new_n_filters, hidden_size=new_hidden).to(device)
    ...
    with torch.no_grad():
        # Conv: preserve existing filters, new filters stay randomly initialized
        new_net.conv[0].weight[:old_nf].copy_(old_net.conv[0].weight)
        new_net.conv[0].bias[:old_nf].copy_(old_net.conv[0].bias)

        # Linear1 weight: shape (hidden_size, flat_size)
        # top-left  (old_h × old_flat)
        new_net.encoder[1].weight[:old_h, :old_flat].copy_(old_net.encoder[1].weight)
        new_net.encoder[1].bias[:old_h].copy_(old_net.encoder[1].bias)
        # top-right (old_h × new_flat-old_flat)
        new_net.encoder[1].weight[:old_h, old_flat:] = 0.0
        # bottom-left (new_h-old_h × old_flat)
        # bottom-right (new_h-old_h × new_flat-old_flat)
        # new hidden biases: random (kept) - bias = weight on constant-1 input,
        # treated as existing to new

        # q_head weight: shape (n_actions, hidden_size)
        # left  (n_actions × old_h)
        new_net.q_head.weight[:, :old_h].copy_(old_net.q_head.weight)
        new_net.q_head.bias.copy_(old_net.q_head.bias)
        # right (n_actions × new_h-old_h)
        new_net.q_head.weight[:, old_h:] = 0.0
...
if __name__ == "__main__":
    ...
    q_network = QNetwork(envs, n_filters=args.initial_filters, hidden_size=args.initial_hidden).to(device)
    optimizer = optim.Adam(q_network.parameters(), lr=args.learning_rate)
    target_network = QNetwork(envs, n_filters=args.initial_filters, hidden_size=args.initial_hidden).to(device)
    target_network.load_state_dict(q_network.state_dict())

    # growth_schedule: list of (global_step_threshold, new_n_filters, new_hidden)
    growth_schedule = [
        (
            args.total_timesteps * k // (args.n_growth_steps + 1),
            args.initial_filters + (_FINAL_FILTERS - args.initial_filters) * k // args.n_growth_steps,
    ...
    for global_step in range(args.total_timesteps):
        ...
        if next_growth_idx < len(growth_schedule):
            grow_at, new_nf, new_nh = growth_schedule[next_growth_idx]
            if global_step >= grow_at:
                feature_split = q_network.q_head.in_features
                n_new = new_nh - feature_split
                target_dirs, reg_obs = compute_growth_directions(
                    q_network, target_network, rb, device,
                    feature_split, n_new,
                    args.batch_size, args.gamma, args.growth_n_batches,
                )
                old_q_network = q_network
                q_network = grow_network(
                    q_network,
                    envs,
                    device,
                    new_nf,
                ...
                writer.add_scalar("growing/n_filters", new_nf, global_step)
                writer.add_scalar("growing/hidden_size", new_nh, global_step)
                print(f"global_step={global_step}: grew network to filters={new_nf}, hidden={new_nh}")
                next_growth_idx += 1
            ...
            if global_step % args.train_frequency == 0:
                ...
                if global_step % 20000 == 0:
                    ...
                    if 0 < feature_split < feature_dim:
                        ...
                        writer.add_scalars(
                            "growing/rank_phi",
                            {"old": rank_old, "new": rank_new},
                            global_step
                        )
                        writer.add_scalars(
                            "growing/rank_ratio_phi",
                            {"old": rank_old / phi.shape[1], "new": rank_new / phi_new.shape[1]},
                            global_step
                        )
                        ...
                        writer.add_scalars(
                            "growing/srank_phi",
                            {"old": sr_old, "new": sr_new},
                            global_step
                        )
                        writer.add_scalars(
                            "growing/srank_ratio_phi",
                            {"old": sr_old / phi.shape[1], "new": sr_new / phi_new.shape[1]},
                            global_step
                        )
                        ...
                        writer.add_scalars(
                            "growing/angles",
                            {"min": cosines.min(), "max": cosines.max(), "mean": cosines.mean()},
                            global_step
                        )
