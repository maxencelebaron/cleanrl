...
class Args:
    ...
    """number of replay buffer batches used to compute growth target
        directions
    """
    growth_replace_idx: int = 0
    """index of the encoder feature column replaced by
    TQ/||TQ|| before growth
    """
    growth_adjust_steps: int = 1000
    """gradient steps to adjust encoder toward Phi* """
    growth_adjust_lr: float = 1e-3
    """learning rate for the pre-growth encoder adjustment"""
    growth_action_idx: int = 0
    """action index used for the Bellman residual direction
    when initializing new neurons
    """
    neuron_init_batch_size: int = 4 * 32
    """batch size for neuron initialization"""
...
def init_new_neurons_gd(
    ...
    n_steps: int,
    lr: float,
    neuron_init_batch_size: int = None,
) -> None:
    if neuron_init_batch_size is None:
        neuron_init_batch_size = 4 * batch_size

    old_h = old_net.q_head.in_features
    n_new = new_net.q_head.in_features - old_h

    data = replay_buffer.sample(neuron_init_batch_size)

    with torch.no_grad():
        x_perm = data.observations.permute(0, 3, 1, 2).float()
        flat_X = new_net.flat(new_net.conv(x_perm))
        phi_old = old_net.encode(data.observations).cpu().numpy()                    # (B, old_h)
        target_max, _ = target_net(data.next_observations).max(dim=1)
        y = (data.rewards.flatten() + gamma * target_max * (1 - data.dones.flatten())).cpu().numpy()
        q_est = old_net(data.observations)[:, action_idx].cpu().numpy()

    mask = (data.actions.squeeze() == action_idx).cpu().numpy().astype(bool)
    n_filt = int(mask.sum())
    if n_filt < old_h + n_new:
        logging.warning(
            f"init_new_neurons_gd: only {n_filt} transitions with action={action_idx} "
            f"(need >= d_old + d_new = {old_h + n_new}). Consider increasing batch_size."
        )
    y       = y[mask]
    q_est   = q_est[mask]
    phi_old = phi_old[mask]
    flat_X  = flat_X[mask]

    r_i = y - q_est
    r_i_norm = float(np.linalg.norm(r_i))
    v_i = r_i / (r_i_norm + 1e-8)

    phi_a_star = np.zeros((n_filt, n_new), dtype=np.float64)
    phi_a_star[:, action_idx] = v_i
    phi_a_star[:, [0, action_idx]] = phi_a_star[:, [action_idx, 0]]
    phi_a_star = _gram_schmidt(phi_a_star, existing_basis=phi_old)
    phi_a_star[:, [0, action_idx]] = phi_a_star[:, [action_idx, 0]]
    phi_a_star_t = torch.tensor(phi_a_star, dtype=torch.float32, device=device)

    flat_X_np = flat_X.cpu().numpy()
    flat_X_aug = np.hstack([flat_X_np, np.ones((flat_X_np.shape[0], 1))])          # (n_filt, flat_size + 1)
    Wb, _, _, _ = np.linalg.lstsq(flat_X_aug, phi_a_star, rcond=None)              # (flat_size + 1, n_new)
    W_new, b_new = Wb[:-1], Wb[-1]                                                  # (flat_size, n_new), (n_new,)
    ...
    with torch.no_grad():
        ...
        new_net.encoder[0].bias[old_h:].copy_(
            torch.tensor(b_new, dtype=torch.float32, device=device)
        )

    opt = optim.Adam(
        [new_net.encoder[0].weight, new_net.encoder[0].bias],
        lr=lr
    )
    for _ in range(n_steps):
        pred_new = F.relu(
            flat_X @ new_net.encoder[0].weight[old_h:, :].T
            + new_net.encoder[0].bias[old_h:]
        )
        loss = F.mse_loss(pred_new, phi_a_star_t)
        opt.zero_grad()
        loss.backward()
        opt.step()
    ...
    with torch.no_grad():
        ...
        new_net.q_head.weight[action_idx, old_h + action_idx] = r_i_norm


def _gram_schmidt(V: np.ndarray, existing_basis: np.ndarray = None) -> np.ndarray:
    n = V.shape[1]
    V = V.astype(np.float64).copy()
    Q = np.empty_like(V)
    R = np.zeros((n, n), dtype=np.float64)

    # orthonormal basis of Phi_old; used to project new columns into its orthogonal complement
    if existing_basis is not None:
        U, _, _ = np.linalg.svd(existing_basis.astype(np.float64), full_matrices=False)
    else:
        U = None

    def _project_out_existing(v):
        if U is not None:
            v = v - U @ (U.T @ v)
        return v

    for j in range(n):
        V[:, j] = _project_out_existing(V[:, j])
        r_jj = np.linalg.norm(V[:, j])

        if r_jj < 1e-10:
            logging.warning(
                f"Gram-Schmidt: column {j} is degenerate (norm={r_jj:.2e}). "
                f"Replacing with canonical vector e_{j} - check encoder (dead ReLU?)."
            )
            v = np.zeros(V.shape[0])
            v[j % V.shape[0]] = 1.0
            v = _project_out_existing(v)
            for jj in range(j):
                v -= (Q[:, jj] @ v) * Q[:, jj]
            ...
            if r_jj < 1e-10:
                ...
                for _ in range(10):
                    v = rng.standard_normal(V.shape[0])
                    v = _project_out_existing(v)
                    for jj in range(j):
                        v -= (Q[:, jj] @ v) * Q[:, jj]
                    r_jj = np.linalg.norm(v)
                    if r_jj >= 1e-10:
                        break
                if r_jj < 1e-10:
                    raise ValueError(f"Cannot orthogonalize column {j}: basis exhausted")
            V[:, j] = v

        R[j, j] = r_jj
        Q[:, j] = V[:, j] / r_jj               # normalize

        for k in range(j + 1, n):
            R[j, k] = Q[:, j] @ V[:, k]        # projection coefficient
            V[:, k] -= R[j, k] * Q[:, j]       # orthogonalize remaining columns

    return Q.astype(np.float32)


def pre_growth_adjust(
    net: QNetwork,
    target_net: QNetwork,
    replay_buffer,
    device,
    ...
    with torch.no_grad():
        x_perm = data.observations.permute(0, 3, 1, 2).float()
        flat_X = net.flat(net.conv(x_perm)).detach()                               # (B, flat_size)
        phi_current = net.encoder(flat_X).cpu().numpy()                             # (B, k)
        target_max, _ = target_net(data.next_observations).max(dim=1)
        y = (data.rewards.flatten() + gamma * target_max * (1 - data.dones.flatten())).cpu().numpy()
    ...
    phi_star = phi_current.copy()
    phi_star[:, replace_idx] = u1
    phi_star[:, [0, replace_idx]] = phi_star[:, [replace_idx, 0]]
    phi_star = _gram_schmidt(phi_star)
    phi_star[:, [0, replace_idx]] = phi_star[:, [replace_idx, 0]]
    phi_star_t = torch.tensor(phi_star, dtype=torch.float32, device=device)

    opt = optim.Adam([net.encoder[0].weight, net.encoder[0].bias], lr=lr)
    for _ in range(n_steps):
        pred = net.encoder(flat_X)
        loss = F.mse_loss(pred, phi_star_t)
        opt.zero_grad()
        loss.backward()
        opt.step()

    with torch.no_grad():
        net.q_head.weight.zero_()
        net.q_head.weight[:, replace_idx] = y_norm
...
def grow_network(
    ...
    Weight initialization rule (function-preserving):
      ...
      - existing neuron → new neuron: random init
      - new neuron → existing neuron: 0
        ...
        if next_growth_idx < len(growth_schedule):
            ...
            if global_step >= grow_at:
                pre_growth_adjust(
                    ...
                    args.batch_size,
                    args.gamma,
                    args.growth_replace_idx,
                    args.growth_adjust_steps,
                    args.growth_adjust_lr,
                )
                ...
                init_new_neurons_gd(
                    ...
                    args.batch_size,
                    args.gamma,
                    args.growth_action_idx,
                    args.growth_adjust_steps,
                    args.growth_adjust_lr,
                    neuron_init_batch_size=args.neuron_init_batch_size,
                )
