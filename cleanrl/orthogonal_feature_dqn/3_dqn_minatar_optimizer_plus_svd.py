...
def pre_growth_optimize(
    ...
    for step in range(n_steps):
        ...
        writer.add_scalar("losses/bellman_residual_during_the_update_step", loss.item(), growth_step + 1 + step)
        optimizer.zero_grad()
        loss.backward()
        optimizer.step()
...
if __name__ == "__main__":
    ...
    writer.add_scalar("growing/hidden_size", args.initial_hidden, 0)

    # growth_schedule: list of (global_step_threshold, new_hidden)
    growth_schedule = [
        (
            args.total_timesteps * k // (args.n_growth_steps + 1),
            args.initial_hidden + (FINAL_HIDDEN - args.initial_hidden) * k // args.n_growth_steps,
        )
        ...
        if next_growth_idx < len(growth_schedule):
            ...
            if global_step >= grow_at:
                ...
                if final_loss <= args.bellman_residual_threshold:
                    ...
                    q_network, svd_singular_values = grow_network_svd(
                        q_network,
                        grow_data,
                        td_target,
                        env=envs,
                    ...
                    writer.add_histogram("growing/svd_singular_values", svd_singular_values, global_step)
