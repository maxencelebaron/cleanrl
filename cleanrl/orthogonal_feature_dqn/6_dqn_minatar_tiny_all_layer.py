...
def grow_layer_gromo(
    ...
    downstream_layer by accumulating TD-loss statistics from the replay buffer.
    The optimizer must be rebuilt after calling this function.
    """
    loss_sum = nn.MSELoss(reduction="mean")

    downstream_layer.init_computation()
    q_network.eval()
    ...
    while n_collected < n_samples:
        ...
        loss = loss_sum(td_target, old_val)
        loss.backward()
        downstream_layer.update_computation()
        n_collected += batch_size

    q_network.train()

    downstream_layer.compute_optimal_updates(maximum_added_neurons=maximum_added_neurons)
    ...
    for global_step in range(args.total_timesteps):
        ...
        if next_growth_idx < len(growth_schedule):
            ...
            if global_step >= grow_at:
                ...
                writer.add_scalar("growing/conv_channels", q_network.conv.out_channels, global_step)
                print(f"global_step={global_step}: grew encoder to hidden={q_network.encoder.out_features}, conv to channels={q_network.conv.out_channels}")
                next_growth_idx += 1
